...
export class AuthModel extends Connection {
  ...
  async login (inputs: InputLogin) {
    const [users] = await this.db.query<User[] & RowDataPacket[]>("SELECT password, email, BIN_TO_UUID(id) as id FROM users WHERE email = ?", [inputs.email])
  ...
  async registrarConfirmando (inputs: InputRegisterConfirmando) {
    ...
    const [email] = await this.db.query<User[] & RowDataPacket[]>("SELECT * FROM users WHERE LOWER(email) = ?", [inputs.email.toLowerCase()])
    ...
    const [cedula] = await this.db.query<User[] & RowDataPacket[]>("SELECT * FROM users WHERE cedula = ?", [inputs.cedula])
  ...
  async registrarCatequista (inputs: InputRegisterCatequista) {
    // Validando que no exista un usuario con el correo recibido
    const [email] = await this.db.query<User[] & RowDataPacket[]>("SELECT * FROM users WHERE LOWER(email) = ?", [inputs.email.toLowerCase()])
    if(email.length > 0) return {error: "Ya existe un usuario con este correo electronico"}
