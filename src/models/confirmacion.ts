...
export class ConfirmacionModel extends Connection {
  ...
  async getConfirmacionActual ({id_parroquia}: { id_parroquia: number }) {
    const [confirmacion] = await this.db.query<RowDataPacket[]>(`SELECT *, BIN_TO_UUID(id) as id FROM confirmaciones WHERE 
      inscribiendo = 1 AND
      activo = 1 AND
      id_parroquia = ?;`, 
      [id_parroquia]
