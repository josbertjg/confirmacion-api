...
export class AuthModel extends Connection {
    ...
    async login(inputs) {
        ...
        if (users.length == 0)
            return { error: 'Correo o contraseña incorrectos, si no tienes una cuenta ponte en contacto con algun catequista de tu parroquia' };
        const user = users[0];
        const isValidPassword = await bcrypt.compare(inputs.password, user.password);
        if (isValidPassword)
            return { message: "Login exitoso" };
        else
            return { error: 'Correo o contraseña incorrectos' };
    }
    async registrarConfirmando(inputs) {
        ...
        if (!!confirmacion.error)
            return { error: confirmacion.error };
        ...
        if (email.length > 0)
            return { error: "Ya existe un usuario con este correo electronico" };
        ...
        if (cedula.length > 0)
            return { error: "Ya existe un usuario con este cedula" };
        // Creando el usuario
        const userId = randomUUID();
        await this.db.query(`INSERT INTO users 
      (id, nombre, apellido, email, cedula, phone, role, born_date, id_parroquia) 
      VALUES (UUID_TO_BIN(?),?, ?, ?, UPPER(?), ?, ?, ?, ?);`, [userId, inputs.nombre, inputs.apellido, inputs.email, inputs.cedula, inputs.phone, "CONFIRMANDO", inputs.born_date, inputs.id_parroquia]);
      ...
      VALUES (UUID_TO_BIN(?),UUID_TO_BIN(?), UUID_TO_BIN(?), ?)`, [confirmandoId, userId, confirmacion.id, inputs.primera_comunion]);
    ...
    async registrarCatequista(inputs) {
        ...
        await this.db.query(`INSERT INTO users 
      (id, nombre, apellido, email, cedula, phone, role, born_date, id_parroquia) 
      VALUES (UUID_TO_BIN(?),?, ?, ?, UPPER(?), ?, ?, ?, ?);`, [userId, inputs.nombre, inputs.apellido, inputs.email, inputs.cedula, inputs.phone, "CATEQUISTA", inputs.born_date, inputs.id_parroquia]);
